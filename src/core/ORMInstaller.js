...
const M1_PACKAGES = {
  javascript: {
    postgresql: { deps: ['pg'], dev: [] },
    mysql: { deps: ['mysql2'], dev: [] },
    sqlite: { deps: ['better-sqlite3'], dev: [] },
  },
};
...
class ORMInstaller {
  ...
  async install(language, database, cwd = process.cwd()) {
    const entry = (M1_PACKAGES[language] || {})[database];
    if (!entry) {
      console.log(
        `Skipping install: ${language}+${database} not supported in M1`
      );
      return;
    }
    const cmd = (pkgs, flag) =>
      pkgs.length ? `npm i ${flag} ${pkgs.join(' ')}` : null;
    const cmds = [cmd(entry.deps, '-S'), cmd(entry.dev, '-D')].filter(Boolean);
    for (const c of cmds) {
      console.log(`> ${c}`);
      await pexec(c, { cwd });
    }
  ...
  async installPackages(language, config) {
    ...
    if (language === 'javascript' || language === 'typescript') {
      ...
      if (config.devPackages.length > 0) {
        ...
        const devCmd = `${packageManager} ${
          packageManager === 'npm' ? 'install' : 'add'
        } ${devFlag} ${config.devPackages.join(' ')}`;
