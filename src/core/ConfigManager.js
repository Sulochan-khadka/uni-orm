...
class ConfigManager {
  ...
  getDefaultPort(database) {
    return { postgresql: 5432, mysql: 3306, sqlite: 0 }[database] ?? 0;
  }

  async writeDotEnv(database, answers = {}) {
    const lines = [];
    if (database === 'sqlite') {
      lines.push(`DB_CLIENT=sqlite`);
      lines.push(`DB_FILE=${answers.DB_FILE || './dev.sqlite'}`);
    } else {
      const defaults = {
        DB_CLIENT: database,
        DB_HOST: answers.DB_HOST || 'localhost',
        DB_PORT: answers.DB_PORT || String(this.getDefaultPort(database)),
        DB_USER: answers.DB_USER || 'user',
        DB_PASSWORD: answers.DB_PASSWORD || 'password',
        DB_NAME: answers.DB_NAME || 'appdb',
      };
  ...
  async ensureGitignore() {
    ...
    let content = fs.existsSync(p) ? await fs.readFile(p, 'utf8') : '';
    if (!content.includes('\n.env')) {
...
config = {
    ...
    'connection': {
        ...
        ${
          database === 'sqlite'
            ? "'filename': os.getenv('DB_FILE', './database.sqlite'),"
            : ''
        }
    },
    ...
    'features': {
        ...
        'cache': ${database === 'redis' ? 'True' : 'False'},
        'transactions': ${
          this.supportsTransactions(database) ? 'True' : 'False'
        }
    }
}`;
  }
...
func GetConfig() Config {
    port, _ := strconv.Atoi(getEnv("DB_PORT", "${this.getDefaultPort(
      database
    )}"))
    ...
    return Config{
        ...
        Connection: Connection{
            ...
            ${
              database === 'sqlite'
                ? 'Filename: getEnv("DB_FILE", "./database.sqlite"),'
                : ''
            }
        },
        ORM: ORM{
